...
def optimize_hyp(
    ...
    def objective(params):
        ...
        for train_I, test_I in rep_kfold.split(X):
            ...
            y_fold_train = y.iloc[train_I].values.flatten()  # type: ignore
            X_fold_test = X.iloc[test_I, :]
            y_fold_test = y.iloc[test_I].values.flatten()  # type: ignore
            # On entraîne une instance du modèle avec les paramètres params
            model = train_model(
                instance=instance,
                training_set=(X_fold_train, y_fold_train),  # type: ignore
                params=params,
            )
    ...
    return fmin(fn=objective, space=search_space, algo=tpe.suggest, max_evals=max_evals)  # type: ignore
...
def auto_ml(
    ...
    """
    X = pd.concat((X_train, X_test))  # type: ignore
    y = pd.concat((y_train, y_test))  # type: ignore
    ...
    for model_specs in MODELS:
        ...
        optimum_params = optimize_hyp(
            ...
            metric=lambda x, y: -f1_score(x, y),  # type: ignore
            max_evals=max_evals,
        )
        print("done")
        # Training the supposed best model with found hyper-parameters
        model = train_model(
            model_specs["model_class"](),  # type: ignore
        ...
        opt_models.append(
            {
                ...
                "params": optimum_params,
                "score": f1_score(y_test, model.predict(X_test)),  # type: ignore
            }
        )
